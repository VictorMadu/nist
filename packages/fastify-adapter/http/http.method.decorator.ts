...
import { Keys } from "ts-util-types";
...
// TODO: Add stricter type for schemas
export function Schema(schema: IMethodMetadata["schema"]) {
  return setMethodMetadata<IMethodMetadata, "schema">("schema", schema);
}

export function BodySchema(bodySchema: Record<string, any>) {
  return setMethodMetadata<IMethodMetadata, "schema.body">(
    "schema.body",
    bodySchema
  );
}
...
export function QueryStringSchema(queryStringSchema: Record<string, any>) {
  return setMethodMetadata<IMethodMetadata, "schema.querystring">(
    "schema.querystring",
    queryStringSchema
  );
}
...
export function ParamsSchema(paramsSchema: Record<string, any>) {
  return setMethodMetadata<IMethodMetadata, "schema.params">(
    "schema.params",
    paramsSchema
  );
}
...
export function HeadersSchema(headersSchema: Record<string, any>) {
  return setMethodMetadata<IMethodMetadata, "schema.headers">(
    "schema.headers",
    headersSchema
  );
}
...
export function ResponseSchema(responseSchema: Record<string, any>) {
  return setMethodMetadata<IMethodMetadata, "schema.response">(
    "schema.response",
    responseSchema
  );
}
